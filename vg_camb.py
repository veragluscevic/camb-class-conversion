...
files = ['test_transfer_z99.dat','output/CDM_class_Vtk.dat','output/CDM_class_Vbackground.dat']
...
np.savetxt('CDM_Tk.dat', np.column_stack((data_camb_sync[:,0], np.abs(data_camb_sync[:,1]), np.abs(data_camb_sync[:,4]),dummy,dummy,dummy,np.abs(data_camb_sync[:,9]),dummy,dummy,dummy,np.abs(vc),np.abs(vb),dummy)),fmt='%15.6e', header=header)